...
def train(loader, model, loss_model, optimizer, sche, epoch,logger,train_loss_list):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        mat_label = label.to('cuda:0')
        #print('label', label.shape)
        inc_V_ind = inc_V_ind.float().to('cuda:0')
        inc_L_ind = inc_L_ind.float().to('cuda:0')
        
        pred_x, x_qk = model(data, inc_V_ind, mode='train')

        loss_Cont = 0
        if args.view_loss:
                loss_Cont = loss_model.contrastive_loss(x_qk, label.cuda(),inc_V_ind.cuda(), inc_L_ind.cuda())
...
def test(loader, model, loss_model, epoch,logger):
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        ...
        data=[v_data.to('cuda:0') for v_data in data]
        inc_V_ind = inc_V_ind.float().to('cuda:0')
        pred,_ = model(data, inc_V_ind, mode='test')
        pred = pred.cpu()
...
def val(loader, model, loss_model, epoch,logger,val_loss_list,val_ap_list,val_hl_list,val_rl_list,val_auc_list,val_oe_list,val_cov_list):
    total_labels = []
    total_preds = []
    model.eval()
    val_loss = 0.
    ...
    for i, (data, label, inc_V_ind, inc_L_ind) in enumerate(loader):
        # data_time.update(time.time() - end)
        data=[v_data.to('cuda:0') for v_data in data]
        label_loss = label.to('cuda:0')
        inc_V_ind = inc_V_ind.float().to('cuda:0')
        inc_L_ind = inc_L_ind.float().to('cuda:0')
        
        pred,x_qk = model(data, inc_V_ind, mode='test')
        
        loss_Cont = 0
        if args.view_loss:
                loss_Cont = loss_model.contrastive_loss(x_qk, label_loss.cuda(),inc_V_ind.cuda(), inc_L_ind.cuda())


        loss_CL = torch.mean(torch.abs((label_loss.mul(torch.log(pred + 1e-10)) \
                                                    + (1-label_loss).mul(torch.log(1 - pred + 1e-10))).mul(inc_L_ind)))
        fusion_loss = loss_CL + loss_Cont * args.beta
    ...
    val_loss_list.append(val_loss.item()/val_steps)
    val_ap_list.append(evaluation_results[0].item())
    val_hl_list.append(evaluation_results[1].item())
    val_rl_list.append(evaluation_results[2].item())
    val_auc_list.append(evaluation_results[3].item())
    val_oe_list.append(evaluation_results[4].item())
    val_cov_list.append(evaluation_results[5].item())
    ...
    return evaluation_results, val_loss_list, val_ap_list, val_hl_list, val_rl_list, val_auc_list, val_oe_list, val_cov_list
...
def main(args,file_path):
    ...
    for fold_idx in range(folds_num):
        ...
        model = get_model(len(d_list), d_list, num_classes=train_dataset.classes_num,embed_dim=512, \
                          dropout=0.2, exponent=2).to(device) # view_inner_dim=32
        
        # print(model)
        loss_model = Loss(args.alpha,args.gamma,device)
        ...
        for epoch in range(args.epochs):
            ...
            train_losses, model, train_loss_list,train_accurate_list= train(train_dataloder,model,loss_model,optimizer,scheduler,epoch,logger,train_loss_list)
            # test_results = test(test_dataloder,model,loss_model,epoch,logger)
            val_results, val_loss_list,val_ap_list,val_hl_list,val_rl_list,val_auc_list,val_oe_list,val_cov_list = val(val_dataloder,model,loss_model,epoch,logger,val_loss_list,val_ap_list,val_hl_list,val_rl_list,val_auc_list,val_oe_list,val_cov_list)
        ...
        if fold_idx == 0:
            ...
            oe_list = val_oe_list
            cov_list = val_cov_list
            
        else:
            train_loss = [train_loss[i] + train_loss_list[i] for i in range(len(train_loss))]
            val_loss = [val_loss[i] + val_loss_list[i] for i in range(len(val_loss))]
            ap_list = [ap_list[i] + val_ap_list[i] for i in range(len(ap_list))]
            hl_list = [hl_list[i] + val_hl_list[i] for i in range(len(hl_list))]
            rl_list = [rl_list[i] + val_rl_list[i] for i in range(len(rl_list))]
            auc_list = [auc_list[i] + val_auc_list[i] for i in range(len(auc_list))]
            oe_list = [oe_list[i] + val_oe_list[i] for i in range(len(oe_list))]
            cov_list = [cov_list[i] + val_cov_list[i] for i in range(len(cov_list))]
    ...
    print('train_loss:', [x / folds_num for x in train_loss])
    print('val_loss:', [x / folds_num for x in val_loss])
    print('ap_list:', [x / folds_num for x in ap_list])
    print('hl_list:', [x / folds_num for x in hl_list])
    print('rl_list:', [x / folds_num for x in rl_list])
    print('auc_list:', [x / folds_num for x in auc_list])
    print('oe_list:', [x / folds_num for x in oe_list])
    print('cov_list:', [x / folds_num for x in cov_list])
    ...
    res_list.extend(['lr='+str(args.lr),'alpha='+str(args.alpha),'beta='+str(args.beta),'gamma='+str(args.gamma)])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default= 45 )  # 45(100) for corel5k, 15(30) for others(no missing)
    parser.add_argument('--sche', type=bool, default=False)
    parser.add_argument('--lr', type=float, default=1e-4) 
    parser.add_argument('--T_max', type=float, default=15)
    ...
    alpha_list = [0.1]  # 0.1 for all
    beta_list = [0.01]  # 0.01 for corel5k, 0.001 for others(all no missing)
    gamma_list = [1] # 
